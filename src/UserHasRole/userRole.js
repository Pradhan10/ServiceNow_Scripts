/*First, we create a GlideRecord of Role table*/
var rolesGR = new GlideRecord('sys_user_has_role');
rolesGR.addQuery('user',gs.getUserID());
rolesGR.query();
/*Check is the user has roles*/
if(rolesGR.next()){
    while (rolesGR.next()){
        var role = rolesGR.role.name; /*Get the name of each role the user has,*/
        gs.debug("HAS ROLE : " + role);
    }
}else{
    gs.debug("The user does not has any roles assigned");
}
